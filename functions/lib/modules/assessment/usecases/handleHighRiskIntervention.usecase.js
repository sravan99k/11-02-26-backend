...
class HandleHighRiskInterventionUseCase {
    constructor(analyticsRepo, userRepo, notificationService) {
    ...
    async execute(studentId, riskCategory, riskPercentage) {
        ...
        if (student && student.schoolId) {
            await this.notificationService.sendEmail("counselor@school.edu", "URGENT: High Risk Student Detected", `High risk detected for student ${student.displayName || studentId}. Please review the dashboard.`);
        }
    }
}
