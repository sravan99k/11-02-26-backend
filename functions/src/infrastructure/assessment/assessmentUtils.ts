 ...
 * @property {number} questionIndex - 0-indexed position of the question
...
/**
 * Creates a Map for O(1) response lookups by question index.
 * 
 * This function builds an efficient lookup structure that enables constant-time
 * retrieval of responses during flag derivation, significantly improving performance
 * over linear searches (O(n²) → O(n)).
 * 
 * @param {AssessmentResponse[]} responsesArray - Array of student responses
 * @returns {Map<number, string|number|string[]>} Map from questionIndex to answer value
 * 
 * @example
 * const responses = [
 *   { questionIndex: 0, answer: 'Yes' },
 *   { questionIndex: 1, answer: ['Math', 'Science'] }
 * ];
 * const map = createResponseMap(responses);
 * console.log(map.get(0)); // 'Yes'
 * console.log(map.get(1)); // ['Math', 'Science']
 * 
 ...
 * 
 * This function performs O(1) lookup using a Map created by `createResponseMap`.
 * Use this in conjunction with `createResponseMap` for optimal performance when
 * deriving flags from multiple questions.
 * 
 ...
 * @param {Map<number, string|number|string[]>} responseMap - Pre-built map of questionIndex → answer
...
export const getAnswerFromMap = (
    ...
    responseMap: Map<number, string | number | string[]>
