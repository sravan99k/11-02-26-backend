...
async function runPhaseExam(studentGrade, currentPhase) {
    ...
    try {
        ...
        console.log("Privacy Statement:");
        console.log(phaseData.major.privacyStatement);
...
function checkForCrisisFlags(responses, assessmentQuestions) {
    ...
    assessmentQuestions.forEach(question => {
        ...
        if (question.textAnalysis && question.textAnalysis.enabled && typeof response === 'string') {
            const keywords = question.textAnalysis.keywords || [];
            ...
            if (foundKeywords.length > 0) {
                criticalFlags.push({
                    ...
                    severity: "HIGH RISK",
                    protocol: "Level 1-2",
                    action: "Immediate counselor review + SMS alert to Mental Health Lead"
                });
            }
        }
    });
